...
def read_single_delivery(client_id,delivery_id):
    client = Client.query.filter(Client.id == client_id).one_or_none()
...
def update_delivery(client_id,delivery_id,delivery):

    client = Client.query.filter(Client.id == client_id).one_or_none()

    if client is not None:

        delivery_check = Delivery.query.filter(Delivery.id == delivery_id).one_or_none()

        if delivery_check is not None:
            delivery_check.description = delivery.get('description')

            db.session.merge(delivery_check)
            db.session.commit()
        
        return print(delivery_schema.dump(delivery_check))
    else:
        abort(404, message=f"Delivery with id: {id}, not found")

def delete_delivery(client_id, delivery_id):
    ...
    if client is not None:
        ...
        return client_schema.dump(delivery)
    else:
        abort(404, message=f"Client with id: {client_id}, not found")


with app.app_context():
    read_single_client(2)